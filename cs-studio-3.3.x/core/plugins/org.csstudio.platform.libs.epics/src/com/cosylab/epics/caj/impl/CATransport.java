...
public class CATransport implements Transport, ReactorHandler, Timer.TimerRunnable {
    ...
    private static final org.apache.logging.log4j.Logger LOG = IsisLog.getLogger(CATransport.class);
	...
	private void closedNotifyContext() {
		...
		ContextVirtualCircuitExceptionEvent cvcee =
	    	new ContextVirtualCircuitExceptionEvent(context, socketAddress.getAddress(), CAStatus.DISCONN);
	...
	protected void processRead(ByteBuffer socketBuffer)
	{
        ...
        LOG.info("Ticket2162: " + "unknown" + " - CATransport processRead end");
	}
	...
	private void responsiveTransport()
	{
		if (unresponsiveTransport)
		{
			...
			for (int i = 0; i < clients.length; i++)
			{
                TransportClient client = clients[i];
                if (client instanceof CAJChannel) {
                    LOG.info("Ticket2162: " + ((CAJChannel) client).getName()
                            + " - CATransport connect change responsive");
                }
                try
				{
	...
	private void unresponsiveTransport()
	{
		if (!unresponsiveTransport)
		{
		    ...
		    ContextVirtualCircuitExceptionEvent cvcee =
		    	new ContextVirtualCircuitExceptionEvent(context, socketAddress.getAddress(), CAStatus.UNRESPTMO);
		    ContextExceptionListener[] listeners = context.getContextExceptionListeners();
			for (int i = 0; i < listeners.length; i++)
			{
				try
     ...
     */
    @Override
    public List<String> getChannelOwners() {
        ...
        for (int i = 0; i < clients.length; i++) {
            TransportClient client = clients[i];
            if (client instanceof CAJChannel) {
                channels.add(((CAJChannel) client).getName());
            }
